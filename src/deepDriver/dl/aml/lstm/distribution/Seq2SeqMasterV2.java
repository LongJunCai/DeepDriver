...
public class Seq2SeqMasterV2 extends Seq2SeqMaster {
	public Seq2SeqMasterV2(int clientsNum, Seq2SeqLSTMBoostrapper boot) throws Exception {
...
//		System.out.println("Prepare to merge Ww from clients");
		...
		if (srvQ2QLSTM.getCfg().isTestQ()) {			
//			batchDeltaWwUpdater.mergeDeltawWs(cfgs, cfg, cfg.getLearningRate(), cfg.getM());
...
//			lSTMWwFresher.freshwWs(cfg);
		...
		batchDeltaWwUpdater.mergeDeltawWs(cfgs, cfg, l, cfg.getM());
		gn.normGradient(cfg, threshold);
		lSTMWwFresher.freshwWs(cfg);
...
//			}
	...
	private void save2File() {
		...
		File f = new File(sf, "data/"+cfgFileName+"_"+a+".m");
		try {
